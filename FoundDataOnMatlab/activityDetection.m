...
load('userTrainingData.mat');


%%
%Step 2: Normalize training data

data = [featureWalk; featureRun; featureIdle];
...
while (i < lastFrame)
    ...
    if useKnn
        ...
        pred = net(currentFeature');
        tempResult = round(pred);
        tempScore = 1- abs(tempResult - pred);
    end
    ...
    if max(tempScore) < confidenceDegreeThreshold|| tempResult ~= lastDetectedActivity 
...
legend([hWalk, hRun, hIdle, hTransition], ...
    'Walking','Running','Idling','Transition'); 
